...
| Quote-to-composer "Add to chat" (v0.8.7; upstream 5eb1c2c1 #13868) | `isCompatibleOrNewer(backendVersion, "0.8.7")` in `ChatUiState.quoteCaptureAvailable`, fails CLOSED on unknown version; also off on assistants endpoints (web `quotesSupported` — they bypass the server-side blockquote merge). Gates only the capture affordance: `message.quotes` DISPLAY stays ungated (self-proving on a received payload) | Selection toolbar carries no "Add to chat" item — a pre-0.8.7 server ignores the request's `quotes` field, so offering capture would silently drop the excerpts | "Add to chat" on the selection toolbar stages excerpt chips above the composer; a fresh send (or composer-origin queue) drains them onto `ChatRequest.quotes`, which the server persists and echoes as `message.quotes`. Regenerate/edit-assistant replay the parent user message's persisted quotes (web `overrideQuotes` parity); continue/edit-user send none. **Composer steers TAKE them from v0.8.8-rc2**, when `POST /chat/steer` began carrying quotes — recovery is the `quotesAccepted` capability echo, deliberately with no version gate on it (see DISCOVERY.md) | `feature/chat/.../viewmodel/ChatUiState.kt` (`quoteCaptureAvailable`) + `feature/chat/src/androidMain/.../components/AddToChatSelectionMenu.kt` + `feature/chat/.../viewmodel/ChatViewModel.kt` (`takePendingQuotes`) | v0.8.7 (drop the version half of the gate; the assistants guard stays) |
| Archive-all conversations (v0.8.8-rc2; `POST /api/convos/archive/all`) | `featureSupport(detected, "0.8.8-rc2").isRuledOut` — **fail-safe suppression, probe-but-do-not-latch**. Offered on `PRESENT` and on `UNKNOWN`, suppressed only on `ABSENT` (a build commit that resolved to a tag below rc2), for the usual reason: a dev build reporting the previous release, or one built past the commit-map pin, is the population most likely to HAVE the route. **Deliberately not latched.** The probe here IS the user's deliberate tap on a confirmed action, not a per-open poll, so re-asking costs one request rather than a loop — and a latch would have to be process-wide (this repository has no account-switch hook), which would carry one server's 404 onto the next account's server and hide a feature that works there. The 404 instead turns the affordance off for the rest of that state's life and says so, which is recoverable by reopening the screen | The Archive All button is not rendered; Clear All and the archived list are unaffected | An "Archive All Conversations" button in Settings → Data, behind a confirmation. On success the server's `archivedCount` is announced and the account's cached rows are bulk-flipped to archived (the archived list is a Room read, so a refetch would paint stale). A 404 reports "this server doesn't support archiving everything at once" rather than a failure | `feature/settings/.../viewmodel/SettingsViewModel.kt` (`archiveAllSupported` in the config combine) → `SettingsUiState.archiveAllSupported` → `feature/settings/.../screen/DataSettingsSection.kt`; `feature/settings/.../viewmodel/delegate/DataManagementDelegate.kt` (`archiveAllChats`) → `core/data/.../repository/ConversationRepositoryImpl.kt` (`archiveAll`) | v0.8.8-rc2 |
| Memory by-id addressing (v0.8.8-rc2; `PATCH`/`DELETE /api/memories/id/:id`) | `featureSupport(detected, "0.8.8-rc2").isRuledOut` — **fail-safe suppression, try-then-fall-back, unlatched**. Suppressed only on `ABSENT`; `UNKNOWN` attempts the by-id route and a 404 falls back to the key-addressed one, which costs one request and is invisible. **`_id` is NOT the signal** — it has been on every list row since long before the routes existed (the query is an unprojected `.lean()` find), so keying off its presence would 404 forever on an rc1 server. Unlatched for the same reason as archive-all: no account-switch hook on this repository, and a process-wide latch would carry one server's verdict onto the next account's | Every read and write is key-addressed, exactly as before. The one thing that cannot work there is reaching a content-filter-redacted entry, whose key the server blanked — mobile reports that honestly rather than letting a blank key resolve to a different row in the same partition | Updates and deletes address the row by `_id`, which is unambiguous across partitions and survives redaction | `core/data/.../repository/MemoryRepositoryImpl.kt` (`byIdHandle` / `updateMemory` / `deleteMemory`) | v0.8.8-rc2 |
| Server-side queued turns (v0.8.8-rc2; `POST`/`GET`/`DELETE /api/agents/chat/queued-turns`) | `supportsFeature(detected, "0.8.8-rc2", landedDate = "2026-08-31")` — **fail-CLOSED, decided by version rather than by probing.** The capability echo alone was not enough: rc1 already stamps `generationCreatedAt` on a run, so every other precondition for server ownership holds there, but it has no queued-turns route — its agents chat router takes the POST as `/:endpoint` (`chat.js:119`) and fails it inside `buildEndpointOption` without a 404, and the reconcile GET fails without one too. No fallback can latch on a response that never arrives, so every follow-up queued mid-run became a row that never sent and blocked the queue behind it. **The gate covers both request sites**: `serverOwnedSpec` (the enqueue) and `refreshQueuedTurns` (the poll that starts on opening an agents conversation and on every foreground) — gating only the enqueue would still send a request per open to a server without the route. `landedDate` (UTC, the route's landing commit `c7f0a8f24`) keeps a dev build that reports the previous release on the server queue once its build commit postdates the route. Behind the gate the capability echo still does its job on a server that has the route: every response carries `capability.supported`, an uncoded 404 or a 501 latches the feature off keyed on the active account, a bounded 4xx other than 408/425 proves the row was never committed, and everything else is reconciled by `clientRequestId` rather than resent | The follow-up queue behaves exactly as it always has: rows stay local and the client drains them itself, one per completed run. Nothing is enqueued and nothing is polled | A follow-up composed during an agent run is handed to the server, which admits and runs it when the current turn completes — so it survives the app being backgrounded or killed, and is visible from another device. The local drain refuses any queue holding a server-owned row, and an admission that consumed the ending run's boundary fences the drain even after its row has left the queue. **This is the only thing preventing a double send:** the server's admission check is gated on `req._isAgentTrigger === true`, which an ordinary send never sets, so there is no server-side guard | `feature/chat/.../viewmodel/ChatViewModel.kt` (`serverOwnedSpec` / `refreshQueuedTurns`, both reading `FeatureGatesState.serverQueueSupported`) ← `core/data/.../repository/QueuedTurnRepositoryImpl.kt` (`classify` / `isUnsupported`) ← `core/model/.../queuedturn/QueuedTurnPolicy.kt` (`isDefiniteQueuedTurnsUnsupported` / `isDefiniteQueuedTurnRejection`) ← `feature/chat/.../viewmodel/delegate/QueuedTurnDelegate.kt` ← `feature/chat/.../viewmodel/delegate/MessageQueueDelegate.kt` (`drainNext`'s two refusals) | v0.8.8-rc2 (drop the version half; the capability echo and the failure classification stay) |
| Scheduled chats (v0.8.8-rc2; `/api/schedules`, `POST /api/schedules/:id/run`) | **No version gate — two capability checks, both of which must hold**, in upstream's order: `SCHEDULES`/`USE`, then `interface.schedules`. **The config half is ABSENT-means-OFF**, the opposite of every other `interface.*` flag on this client, because the feature is experimental and an admin opts in explicitly (`absent`/`null`/`false` → off; `true` → on; `{}` → **on**; `{use:false}` → off). It is read through the single dedicated resolver `isSchedulesEnabled` — **do not route it through a generic interface-flag helper**, which would default it ON and surface a screen whose create and run operations the backend rejects. The boolean form is a RUNTIME FEATURE DISABLE, not a permission denial (`RUNTIME_CONFIG_INTERFACE_FIELDS`), so a disabled server is never reported as missing permission. **Enablement is never probed**: the router mounts unconditionally, so a 404 proves only that the server predates the feature. The permission half deliberately diverges from upstream's fail-closed `useHasAccess` and follows this codebase's own split instead — permissive on USE (list visibility), fail-CLOSED on CREATE (every mutating schedule route needs USE **and** CREATE). That is safe precisely because the config half is fail-closed: a permissive USE cannot open the feature on a server that has not opted in | No drawer entry, no screen, no requests. Identical to every server before v0.8.8-rc2 | A Scheduled chats entry in the drawer opens a list and an editor. The editor mirrors the server-resolved policy served with the list — it refuses a cadence below `minIntervalMinutes` before submit, hides Create at `maxPerUser`, offers no project picker when the deployment pins one, and says to create a project when `requireProject` has nothing to choose. Create, edit, delete and run-now appear together or not at all | `core/model/.../config/InterfaceConfig.kt` (`isSchedulesEnabled`) ← `feature/conversations/.../drawer/DrawerViewModel.kt` (the drawer entry's two halves) and `feature/schedules/.../viewmodel/SchedulesListViewModel.kt` (the screen's) | Never — there is no version threshold to remove. Both capability checks are the contract |
| Read-only subagent child threads (v0.8.8-rc2; `GET /api/convos/:id/subagents`, `GET …/subagents/:threadId`) | `featureSupport(detected, "0.8.8-rc2").isRuledOut` — **fail-safe suppression, and deliberately UNLATCHED**. Suppressed only on `ABSENT` (a build commit that resolved to a tag below rc2); `UNKNOWN` is asked, because an unplaceable server is the population most likely to have the routes. **Nothing latches off a 404 here, and that is the point of the row.** The index answers 404 for three unrelated conditions behind one body (`{"error":"Conversation not found"}`): the conversation is missing, is not the caller's, or **is itself a subagent thread**. None is "the route is absent", so a latch would disable the feature account-wide the first time a user opened a conversation that simply has no children. An old server's `apiNotFound` body differs only by KEY (`message` vs `error`), and leaning on response prose is the same fragility rejected for the queued-turn latch. Every 404 therefore means exactly "no child view for this conversation" and is not remembered. The cost on an unplaceable old server is one GET — and only if a user opens the sheet, since the whole fetch is lazy | The "View thread" row is absent from the subagent trace card, and no request is issued. That suppression is at the AFFORDANCE (`FeatureGatesState.subagentThreadsSupported` → `LocalSubagentThreadsAvailable`), not only inside the repository: `on_subagent_update` is v0.8.6, so a 0.8.6/0.8.7 server draws the very trace cards the row hangs off, and gating only the request would leave a row that answers "unavailable" on exactly those servers. The live trace and the persisted `subagent_content` are unaffected — a different mechanism, two releases older | A sheet off the trace card lists the conversation's children (including ones spawned by an event binding, which have no tool call and render nowhere else) and shows one child's activity, messages and history across turns, with cursor paging | `core/data/.../repository/SubagentRepositoryImpl.kt` (`isRuledOutForServer`) ← `feature/chat/.../viewmodel/SubagentThreadsViewModel.kt` (`openFor`, and the unlatched 404) ← `feature/chat/.../components/SubagentThreadsSheet.kt` | v0.8.8-rc2 (drop the version half; the per-conversation 404 handling stays regardless) |
| Conversation trace viewer (v0.8.8-rc3; `GET /api/traces/:id/availability`, `…/records`, `…/records/:recordId`) | **Two gates, and the version one is the belt rather than the braces.** The live gate is `interface.traceViewer`, read through the dedicated `isTraceViewerEnabled` — an **object-only, default-OFF** section whose `enabled` must be explicitly `true`, so `{}` is **off**. That is NOT the `interface.schedules` rule despite the identical-looking shape (`schedules: {}` is **on**), and routing one through the other's resolver compiles; `TraceViewerEnabledTest` pins the divergence. On top of it sits `featureSupport(detected, "0.8.8-rc3").isRuledOut` — fail-safe suppression, `UNKNOWN` is asked — which in practice never decides anything on a real server: rc1/rc2's `interfaceSchema` is a plain `z.object` with no `.passthrough()`, so a pre-rc3 server strips `traceViewer` before serving `/api/config` and the flag is already absent. It earns its place only for a build that reports a tag it predates. **Nothing latches**, because there is nothing to latch on: `/availability` answers `200 {available:false}` for every refusal including the feature being switched off, so no response shape means "this deployment lacks the routes" | No entry in the chat overflow menu, and no request is issued — not even `/availability`. The suppression is at the affordance, which is also where the conversation-scoped half lives: the gate holds the conversation id whose availability was confirmed, not a boolean, so leaving a traced conversation cannot leave the entry point showing across the round trip that resolves the next one | A "View trace" item opens a sheet of the conversation's turns, newest first, each record's kind, model, duration and status nested under its parent, with cursor paging and a per-record detail. Availability is re-read when a run settles — the turn it just added is what can make a trace readable for the first time | `core/model/.../config/InterfaceConfig.kt` (`isTraceViewerEnabled`) + `core/data/.../repository/TraceRepositoryImpl.kt` (`isRuledOutForServer`) ← `feature/chat/.../viewmodel/ChatViewModel.kt` (`observeTraceAvailability`) → `FeatureGatesState.traceViewerConversationId` → `feature/chat/.../components/ChatOverflowMenu.kt` | v0.8.8-rc3 (drop the version half; the `interface.traceViewer` check is the contract and stays) |
...
- **v0.8.8-rc3 sync:** three ANNOUNCED-capability gates were added and deliberately given **no catalog
  ...
  (rc2), memory by-id (rc2), read-only subagent child threads (rc2), the conversation trace viewer
  (rc3), and server-side queued turns (rc2) — the last with its capability echo kept behind the version
  check, because rc1 fails the route without the 404 the echo needs. One feature landed with **no
  version gate at all**: scheduled chats is two capability checks — `SCHEDULES`/`USE` and an
  absent-means-OFF `interface.schedules`. The trace
