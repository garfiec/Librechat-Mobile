...
| Server-side queued turns (v0.8.8-rc2; `POST`/`GET`/`DELETE /api/agents/chat/queued-turns`) | **No version gate at all — capability echo only.** Every response carries `capability.supported`, and every failure is classified by what it PROVES: an uncoded 404 (an older server falls through the agents router to `apiNotFound`, which answers `{"message":"Endpoint not found"}` before the SPA fallback) or a 501 latches the feature off for that server, keyed on the active account so one server's verdict is never carried onto the next. A bounded 4xx other than 408/425 proves the row was never committed; everything else — 5xx, a timeout, a dropped connection — leaves the outcome UNKNOWN and is reconciled by `clientRequestId` rather than resent. Stacking a version threshold on top would suppress the feature on exactly the servers most likely to have it (a dev build reporting the previous release, or one built past this app's commit-map pin), and the announced capability already answers the question without one | The follow-up queue behaves exactly as it always has: rows stay local and the client drains them itself, one per completed run. Nothing is enqueued, nothing is polled after the first refusal | A follow-up composed during an agent run is handed to the server, which admits and runs it when the current turn completes — so it survives the app being backgrounded or killed, and is visible from another device. The local drain refuses any queue holding a server-owned row, and an admission that consumed the ending run's boundary fences the drain even after its row has left the queue. **This is the only thing preventing a double send:** the server's admission check is gated on `req._isAgentTrigger === true`, which an ordinary send never sets, so there is no server-side guard | `core/data/.../repository/QueuedTurnRepositoryImpl.kt` (`classify` / `isUnsupported`) ← `core/model/.../queuedturn/QueuedTurnPolicy.kt` (`isDefiniteQueuedTurnsUnsupported` / `isDefiniteQueuedTurnRejection`) ← `feature/chat/.../viewmodel/delegate/QueuedTurnDelegate.kt` ← `feature/chat/.../viewmodel/delegate/MessageQueueDelegate.kt` (`drainNext`'s two refusals) | Never — there is no version threshold to remove. The capability echo and the failure classification are the contract |
