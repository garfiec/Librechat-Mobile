...
Kept in the repo so it stays in sync with the release process it describes. The version
placeholders and the *Reproducibility* section below can only be filled by an actual
verified release — see *Before filing*. **Do not file this as-is.**
...
<!-- FILL IN before filing: rebuild the release in `registry.gitlab.com/fdroid/fdroidserver`,
     then replace this whole paragraph with the `apksigcopier compare` result. Do not claim
     verification here until that run has actually happened. -->

Three things were fixed specifically to make this work:
...
Releases before those fixes are permanently unreproducible and are not offered for inclusion.
...
- Gradle dependencies are Apache-2.0 or MIT except `desugar_jdk_libs`, which is
  GPLv2-with-Classpath-Exception. The vendored web assets are covered separately below.
- **Prebuilt JavaScript in `assets`.** The artifact, diagram and math views are WebViews,
  and everything they execute is vendored into the repo under
  `feature/chat/src/commonMain/composeResources/files/web/` rather than fetched from a CDN
  — nine libraries (KaTeX, mermaid, marked, marked-highlight, highlight.js, Tailwind,
  @babel/standalone, React, react-dom), each as the publisher's own released browser build.
  They are minified because that is the form the publishers ship; none is generated by this
  project. `scripts/web-assets.json` is a registry recording, per library, the exact
  version, licence, upstream source and why that pin is held, and
  `scripts/vendor-web-assets.py --check` verifies the tree against a sha256 lock in CI, so
  a file cannot be edited in place. Each library's own LICENSE is vendored alongside it.
  Licences are MIT except highlight.js, which is BSD-3-Clause. One caveat worth stating
  plainly: Tailwind 3.4.17 published no browser build to npm, so `tailwind.min.js` comes
  from Tailwind Labs' own immutable version URL rather than a registry tarball; its source
  is `tailwindlabs/tailwindcss` at tag `v3.4.17` (MIT). Happy to discuss if bundled
  publisher builds are a problem for inclusion.
...
2. Paste the `apksigcopier compare` result into *Reproducibility*, replacing the
   placeholder comment — that section currently claims nothing.
3. Re-run the container probe against the release being filed and correct *Toolchain* if
   anything moved; it is filled in from an earlier probe, not from that release.
